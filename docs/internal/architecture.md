...
- **Index-lag reconciliation (Drive Path A):** `loadIndex()` runs `reconcileMainCorpus()` — a cheap `readdir` that ADDS any top-level `.md` present on disk but omitted by the index (reads only the missing files; normally zero). Covers the window where a Drive-synced note has landed locally before the synced `.wren-index.json` caught up. The index stays authoritative for notes it lists.
...
- **Drive placeholders:** in Google Drive for Desktop *stream* mode a `.md` may be a 0-byte / unhydrated placeholder. Scans skip empty/whitespace files (with a stderr warning) so they never become junk entries; a read that *throws* during a scan is caught in `parseNoteFile`; `readNoteByWrenId` wraps its read and throws `NoteUnreadableError` with a mirror-mode hint. See `docs/DRIVE.md`.
